...
var sperr = document.getElementById('sperr'); //错误信息提示
// 控制台验证
...
jisuan.addEventListener('click', function() {
  ...
  // parseFloat转换参数成为数值，如果参数是数值类型就会成功转转换，否则返回
  // isNaN判断参数是不是非数，不是就返回true真，是数就返回false假
  ...
  if (isNaN(num02)) {
    sperr.innerHTML = '加法的第二个输入的不是数';
    return;
  }
  // 第三个数
  var num03 = parseFloat(n03.value);
  if (isNaN(num03)) {
    sperr.innerHTML = '减法的第一个输入的不是数';
    return;
  }
  // 第四个数
  var num04 = parseFloat(n04.value);
  if (isNaN(num04)) {
    sperr.innerHTML = '减法的第二个输入的不是数';
    return;
  }
  // 第四步，计算显示结果
  // 执行到这里，说明数值都输入了
